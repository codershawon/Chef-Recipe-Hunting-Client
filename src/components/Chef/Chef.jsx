...
const Chef = ({ chef }) => {
  const { id, name, picture, experience,recipes, likes } =
    chef;
    console.log(chef)
  return (
   <div className="flex">
    ...
    <div className="card-body leading-7">
      ...
      <p className="border-s-4  border-pink-500 rounded"><span className="ml-2 font-bold">Number Of Recipes:</span> <span className="text-[#757575] font-semibold">{recipes}</span></p>
      <div className="card-actions justify-end">
       ...
       <Link to={`/foodRecipes/${id}`}> <button className="btn btn-outline btn-secondary -mt-2">View Recipes</button></Link>
      </div>
    </div>
  </div>
   </div>
